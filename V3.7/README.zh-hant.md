...
MoziAI-35B-A3B-MOE 是由中國財經大V陳雨墨團隊開發的本地開源多模態AI大模型（增強金融領域、支持視覺、工具調用、消費級顯卡本地部署），moziAI-35B 基於開源底座 Ornith-1.5-35B-A3B（Qwen3.5-35B-A3B / Qwen3.6-35B-A3B 架構，MIT + Apache-2.0 (Qwen3.5/3.6 & Gemma 4 components) 許可），結合陳雨墨團隊自主研發的：（金融數據 + 金融領域能力 + 訓練方法 + 七維思考體系 + 智能體LOOP機制 + 混合量化算法 MoziSmartBit）開發而成。通過自研的 MoziSmartBit 智能量化 技術，將350億參數MoE模型壓縮至約 15.5 GB，比常規Q4_K_M量化約22+GB的模型體積小了6.5G（約30%）；在精度與體積間取得最優平衡，實現幾乎≈FP16 的 99%的精度質量。通過自研的 MoziSmartBit 智能量化 技術，將350億參數MoE模型壓縮至約 15.5 GB，比常規Q4_K_M量化約22+GB的模型體積小了6.5G（約30%）；在精度與體積間取得最優平衡，實現幾乎≈FP16 的 99%的精度質量。
...
| 項目     | 參數                                                                                 |
| ------ | ---------------------------------------------------------------------------------- |
| 底座模型   | Ornith-1.5-35B-A3B（Qwen3.5-35B-A3B / Qwen3.6-35B-A3B 架構，MIT + Apache-2.0 (Qwen3.5/3.6 & Gemma 4 components) 許可證                       |
...
| context\_length   | 131072                           | 256K 長上下文              |
...
| 24 GB  | 256K 滿配 | q4\_0 | 完美支持 | 視覺+128K長上下文,僅佔顯存\~20.4GB，顯存餘量\~3.6GB |
| 32 GB+ | 256K 滿配 | q4\_0 | 完美支持 | 視覺+128K長上下文，顯存餘量充足\~10GB，最強配置       |
**NVIDIA 顯卡參考表**
...
MoziAI 基於 ornith-ai/Ornith-1.5-35B-A3B 底座微調、蒸餾與二次開發。MoziAI 在底座基礎上針對金融垂直領域優化，在金融問答、量化程式編寫、工具調用等場景下表現更出色。以下為多模型對比（MoziAI-35B 通用能力與底座Ornith-1.5-35B-A3B 一致）底
...
- **視覺文件**：`mmproj/35B/moziAI-35B-mmproj-BF16-V1.0.gguf`（約 903 MB，BF16 精度優
...
├── moziAI-35B-mmproj-BF16-V1.0.gguf  # 視覺投影（可選）
