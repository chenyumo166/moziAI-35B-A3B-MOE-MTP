...
[English](README.en.md) | 简体中文 | [繁体中文](README.zh-hant.md) | [日本语](README.ja.md) | [한국어](README.ko.md) | [हिन्दी](README.hi.md) | [Deutsch](README.de.md) | [Français](README.fr.md) | [Nederlands](README.nl.md) | [Italiano](README.it.md) | [Русский](README.ru.md)
...
MoziAI-35B-A3B-MOE 是由中国财经大V陈雨墨团队开发的本地开源多模态AI大模型（强化金融领域、支持视觉、工具调用、消费级显卡本地部署），基于 Ornith-1.5-35B-A3B（Qwen3.5-35B-A3B / Qwen3.6-35B-A3B** 架构）底座进行二次开发/微调/蒸餾。透过自研的**MoziSmartBit 智能量化** 技术，将350亿参数MoE模型压缩至约 **15.5 GB**，在精度与体积间取得最优平衡，实现幾乎≈FP16 的 99%的精度质量化



本模型除了保留通用AI大模型的能力，重点优化：金融垂直领域的应用，包括金融问答、量化程式编写、工具调用、通用程式编写等AI模型能力时



模型研发者陈雨墨常把本模型用于本地金融数据分析、量化策略研发、市場调研、任何的文章编写、整体项目推进、通用程式编写，openclaw/hermes执行256K上下文的任务。因本地消费级显卡可部署使用，節約大量云端token成本，实现X24小时token自由並且确保本地数据隐私与安全策
...
- **MoziSmartBit 智能量化**：自研的智能量化技术，精度与体积最佳平衡，模型幾乎无损压缩至約 **15.5 GB**

- **消费级部署**：20GB / 4GB显存以上的家用消费级显卡即可本地部署，支持最优256K 长上下文推理

- **多语言支持**：支援201 种语言和方言，中文能力特别优化，兼顾英语、日语、韓语、德语、法语、西班牙语、葡萄牙语等主流语言
...
| 完整资讯  | 提供未经过濾的完整资讯，适合研究和分析場景|
| 本地私有  | 本地部署意味著数据完全私有，不受云端审查  |
**适用場景**：学术研究、深度分析、自由讨论、不受限的AI对话格
...
| 能力领域  | 说明                                         |
| ----- | ------------------------------------------ |
| 市場分析  | 宏观/微观经濟解读、A 股/港股/美股/商品/加密货币行情与逻辑梳理         |
...
> MoziAI V3.7 採用 MoziSmartBit 智能量化方案，在保持约99% 精度的同时，将350亿参数MoE模型压缩至约 15.5 GB，压缩比达4.5x，兼顾推理质量与部署门檻，更适合消费级显卡本地部署的
...
传统量化方案对所有层使用统一精度，而陈雨墨团队自研的**MoziSmartBit 智能量化** 针对 MoE 模型的结构特点，採用智能差异化量化策略，在体积与精度间取得最优平衡，模型质量高于 Q4\_K\_M 格式，同时体积仅占15.5 GB，压缩比达 4.5x。
...
传统量化方案对模型所有部分统一压缩，往往导致精度损失明显。MoziSmartBit 智能量化採用自研的智能压缩策略，**在极小的精度损失下实现大幅体积缩小**。- **量化精度损失极小**：训练增益> 量化损失，训练后的MoziAI-35B 在金融领域文本上下PPL 优于训练前的 bf16 底座，降低了同类 AI 模型的幻觉与困惑。- **模型体积压缩至 4.5 倍**：从 FP16（~70 GB 压缩至\~15.5 GB，也大幅小于Q4\_K\_M的\~21 GB，大幅降低显存与存储门檻
...
> 💡 **提示**：只要显存滿足以上要求即可使用，不限品牌型号，支援NVIDIA / AMD / Intel 各品牌獨立显卡，也支持上下128GB 统一记忆体的核显 CPU。> 💡 **提示**：上下文越长，佔用显存越多。如果出现显存不足（OOM），请逐步降低 `-c` 参数值。使用`--fit on` 参数可让 llama.cpp 自动调整层数适配显存的
...
MoziAI 基于 deepreinforce-ai/Ornith-1.5-35B-A3B 底座微调、蒸餾与二次开发。MoziAI 在底座基礎上针对金融垂直领域优化，在金融问答、量化程式编写、工具调用等場景下表现更出色。以下为多模型对比（MoziAI-35B 通用能力与底座Ornith-1.5-35B-A3B 一致）底
...
> MoziAI-35B 通用能力基准分数与底座Ornith-1.5-35B-A3B 一致。金融垂直领域为 MoziAI 的核心优化方向，在财报解读、量化策略、风控合规、agent管理工具调用等場景下表现显著优于通用模型。Gemma4 / Qwen3.6 数据为官方公开评测结果取
...
> 💡 **下载提示**：请点击上方连结进入 HuggingFace 仓库，在 **"Files and versions"** 标签页下文V3.7 目录下的所有文件（主模型、视觉投影、聊天模板），确保三个文件放在同一目录下文
...
完整的推荐配置启动命令请参考上下[llama.cpp 启动命令](#llamacpp-启动命令) 章節約
...
金融AI大模型、AI大模型、本地开源模型、端側模型、量化程式编写、MoziSmartBit、智能量化、GGUF量化、MoE模型、本地开源大模型、本地部署、金融AI、工具调用、Agent、llama.cpp、Ollama、GGUF、Uncensored（去审核）、无审查、免审核、自由输出、Q3\_K\_M、Q4\_K\_M、Q5\_K\_M、Q6\_K、Q8\_0、Ornith-1.5-35B-A3B、Qwen3.5-35B-A3B、Qwen3.6-35B-A3B、金融垂直领域、开源模型
