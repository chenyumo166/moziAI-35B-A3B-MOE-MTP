...
MoziAI-35B-A3B-MOE 是由中國財經大V陳雨墨團隊開發的本地開源多模態AI大模型（增強金融領域、支持視覺、工具調用、消費級顯卡本地部署），moziAI-35B 基於開源底座 Ornith-1.5-35B-A3B（Qwen3.5-35B-A3B / Qwen3.6-35B-A3B 架構，MIT 許可），結合陳雨墨團隊自研的金融數據 + 金融領域能力 + 訓練方法 + 七維思考體系 + 智能體LOOP機制 + 混合量化算法 MoziSmartBit 開發而成。通過自研的 MoziSmartBit 智能量化 技術，將350億參數MoE模型壓縮至約 15.5 GB，比常規Q4_K_M量化約22+GB的模型體積小了6.5G（約30%）；在精度與體積間取得最優平衡，實現幾乎≈FP16 的 99%的精度質量。



本模型除了保留通用AI大模型的能力，重點優化：金融垂直領域的應用，包括金融問答、量化程式編寫、工具調用、通用程式編寫等AI模型能力時
...
- **消費級顯卡可證*：原本需要高端顯卡的 35B MoE 大模型，現在 20GB\~24GB 顯存即可流暢部署
...
> 💡 **下載提示**：請點擊上方連結進入 HuggingFace 倉庫，在 **"Files and versions"** 標籤頁下文V3.7 目錄下的所有文件（主模型、視覺投影、聊天模板），確保三個文件放在同一目錄下文
