...
- **MoziSmartBit 智能量化**：35B MoE 压缩至 15.9 GB（4.5x 压缩比），精度 ~99%
...
**分层许可模式**：✅ 免费商用使用、复制和分发 / ❌ 二次开发、转售售卖或再许可 **仅针对 MoziAI 原创增量部分**（上游组件保留原始 Apache 2.0 / MIT 许可） / 📋 保留全部上游版权声明（[`NOTICE`](../NOTICE)）和本许可证。详见 [`LICENSE`](../LICENSE)、[`LICENSE-APACHE`](../LICENSE-APACHE)、[`NOTICE`](../NOTICE) 和 [`MODIFICATIONS.md`](../MODIFICATIONS.md)。模型输出仅供参考，不构成投资建议。
