...
# MoziAI-V3.6-35B-A3B-MOE - 無料でローカルデプロイできる小型高性能マルチモーダルAI

Language / 言語選�? 
[简体中文](README.zh.md) | [繁體中文](README.zh-hant.md) | 日本�?| [한국어](README.ko.md) | [हिन्दी](README.hi.md) | [English](README.en.md) | [Deutsch](README.de.md) | [Français](README.fr.md) | [Nederlands](README.nl.md) | [Italiano](README.it.md) | [Русский](README.ru.md)
...
モデルサイズが大きく（約 15.5 GB）、複数のコミュニティプラットフォームでホストされています�?
...
> 💡 **ダウンロードのヒン�?*: 上記リンクをクリックして HuggingFace リポジトリに移動し�?*「Files and versions�?* タブ�?V3.6 ディレクトリ配下のすべてのファイルをダウンロードしてください（メインモデル、ビジョン射影、チャットテンプレート）�? つのファイルすべてを同じディレクトリに配置してください�?

### ⚠️ 重要：ビジョン機能に�?mmproj ファイルが必要で�?

本モデルはマルチモーダルビジョンをサポートしています�?*ビジョン射影ファイル（mmproj�?* はバージョンディレクトリに含まれています�?

- **ビジョンファイル**: `moziAI-V3.6-35B-uncensored-heretic-mmproj-BF16.gguf`（約 903 MB、BF16 精度�?
- **配置場所**: GGUF モデルファイルと同じバージョンディレクト�?
- **読み込み**: llama-server 起動時に `--mmproj` フラグを使用して読み込み

> ビジョンファイルがない場合、モデル�?*画像理解機能を失�?*、テキストのみの会話のみが可能になります�?

### ⚠️ 重要：チャットテンプレートファイルを必ず読み込んでくださ�?

本モデルは専用のチャットテンプレートを使用しています�?*読み込まないと、対話形式のエラー、推論チェーンの破綻、応答品質の大幅な低下が発生します�?* テンプレートファイルはバージョンディレクトリに含まれています�?

- **テンプレートファイル**: `moziAI-V3.6-35B-chat-template.jinja`（数 KB、jinja 形式�?
- **配置場所**: GGUF モデルファイルと同じバージョンディレクトリに配置
- **読込方法**: llama-server 起動時に `--chat-template-file` パラメータで指定

> チャットテンプレートを読み込まないと、モデルがシステムプロンプト、ユーザーメッセージ、思考ブロックを正しく認識できず、出力形式が乱れたり推論能力が低下したりする可能性があります�?

### llama.cpp 起動コマンド�?0GB以上 GPU �?256K コンテキストを有効にする推奨設定�?

> 備考：VRAM �?20GB 未満の場合は、`-c 262144` のコンテキストサイズ 262144 を減らしてください�?
...
HuggingFace / ModelScope から V3.6 ディレクトリ配下のすべてのファイルをダウンロード：

```
V3.6/
├── moziAI-V3.6-Qwen3.6-35B-A3B-Ornith-MoziSmartBit-Q4_K_M-Uncensored.gguf      # メインモデル（必須）
├── moziAI-V3.6-35B-uncensored-heretic-mmproj-BF16.gguf  # ビジョン射影（オプション�?
└── moziAI-V3.6-35B-chat-template.jinja                  # チャットテンプレート（必須！ないと対話形式エラー�?
```

> ⚠️ **チャットテンプレートは必須ファイ�?*であり、オプションではありません。本モデルにはカスタムの対話形式（推論チェーン・思考ブロックを含む）があり、テンプレートがないと出力形式が乱れ、推論能力が無効になります。必ずダウンロードして起動時に読み込んでください�?

### 2. 推論サービスの開�?

完全な推奨設定の起動コマンドについては、上記の **llama.cpp 起動コマンド** の章を参照してください�?
...
> ビジョン機能を追加するに�?`--mmproj V3.6/moziAI-V3.6-35B-uncensored-heretic-mmproj-BF16.gguf` を追加してください�?

### 3. 利用開始

ブラウザ�?`http://localhost:8080` を開いてチャットを開始します�?
...
> MoziAI V3.6はMoziSmartBit Intelligent Quantizationを採用し、約99%の精度を維持しながら35BパラメータのMoEモデルを�?5.5 GB（約4.5倍の圧縮率）に圧縮し、推論品質とコンシューマGPU向けデプロイ可能性のバランスを実現しています�?
...
|------|-----------------|-------------|---------------|------|
| 20 GB | 128K | q4_0 | サポートあり | モデ�?vision �?6.4GB、実測で200K+vision使用時のVRAM消費は約19.5GB |
...
MoziAI�?*deepreinforce-ai/Ornith-1.0-35B**からファインチューニングされています。MoziAIはベースモデルの上で金融垂直ドメインの最適化が施されており、金融Q&A、クオンツプログラミング、tool callingシナリオにおいて優れた性能を発揮します。MoziAI-35Bの汎用能力はOrnith-1.0-35Bベースモデルと一致しています�?
...
financial AI LLM, ローカルオープンソースモデル, エッジサイドモデ�? クオンツプログラミン�? MoziSmartBit, インテリジェント量子�? GGUF量子�? MoEモデ�? ローカルオープンソースLLM, ローカルデプロイ, 金融AI, tool calling, Agent, llama.cpp, Ollama, GGUF, Uncensored, 検閲なし, 自由出力, 制限なし, Q3_K_M, Q4_K_M, Q5_K_M, Q6_K, Q8_0, Ornith-1.0-35B, Qwen3.5, Qwen3.6, 金融垂直ドメイン, オープンソースモデル
