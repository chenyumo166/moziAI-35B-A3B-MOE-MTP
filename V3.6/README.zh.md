...
# moziAI-13.7-35B-A3B-A3B-MOE-MTP-Uncensored - 可免费本地部署的小而强的多模态AI模型
...
本模型使用专属的聊天模板（chat-template），**不加载将导致对话格式错误、推理链失效、回复质量大幅下降**。聊天模板文件已包含在版本目录中：

- **模板文件**：`moziAI-V3.6-35B-chat-template.jinja`（约 5 KB，jinja 格式）
- **放置位置**：与 GGUF 模型文件放在同一版本目录
- **加载方式**：启动 llama-server 时通过 `--chat-template-file` 参数加载

> 不加载聊天模板，模型可能无法正确识别系统提示、用户消息和思考区块，导致输出格式混乱或推理能力下降。

### llama.cpp 启动命令（20G+显卡开256K上下文的推荐配置）

> 备注：如显存低于 20G，则减少 `-c 262144` 的上下文设置参数 262144。
...
> ⚠️ **聊天模板是必选文件**，不是可选。本模型有自定义的对话格式（含推理链/思考区块），缺失模板将导致模型输出格式混乱、推理能力失效。请务必下载并在启动时加载。

### 2. 启动推理服务

完整的推荐配置启动命令请参考下文 [llama.cpp 启动命令](#llamacpp-启动命令) 章节。
...
<tr>
<td>无审查限制</td>
<td>不会拒绝任何话题，包括敏感、争议性内容</td>
</tr>
<tr>
<td>自由输出</td>
<td>不受安全策略约束，可生成任何类型的回复</td>
</tr>
<tr>
<td>完整信息</td>
<td>提供未经过滤的完整信息，适合研究和分析场景</td>
</tr>
<tr>
<td>本地私有</td>
<td>本地部署意味着数据完全私有，不受云端审核</td>
</tr>
</tbody>
</table>
...
<tr>
<td>市场分析</td>
<td>宏观/微观经济解读、A 股/港股/美股/商品/加密货币行情与逻辑梳理</td>
</tr>
<tr>
<td>财务与研报</td>
<td>财报关键指标解读、研报摘要提取、估值与盈利预测辅助</td>
</tr>
<tr>
<td>风控与合规</td>
<td>产品风险评估、投资建议合规提示、金融监管政策解读</td>
</tr>
<tr>
<td>量化与策略</td>
<td>量化策略思路设计、金字塔（Pyramid/PEL）量化、回测逻辑、因子构建与工具调用</td>
</tr>
<tr>
<td>工具调用</td>
<td>可接入实时行情、数据库、研报检索等金融数据</td>
</tr>
</tbody>
</table>
...
<tr>
<td>参数规模</td>
<td>350亿（35B）MoE 架构，256 个路由专家 + 1 个共享专家，每个 token 激活 8 个专家</td>
</tr>
<tr>
<td>量化方式</td>
<td>采用自研 MoziSmartBit 智能量化算法 + GGUF 标准格式</td>
</tr>
<tr>
<td>上下文长度</td>
...
<tr>
<td>最低显存要求</td>
<td>20GB显存以上的家用消费级显卡（如 RTX 3060 12G 需搭配 CPU 卸载，RTX 4060 Ti 16G 等），推荐 24 GB（含视觉 + 长上下文）</td>
</tr>
<tr>
<td>推理框架</td>
...
<tr>
<td>推理速度</td>
<td>通过算法优化，AMD Radeon AI PRO R9700 显卡可达 140+token/s / AMD Ryzen AI Max+ 395 核显可达 70+token/s，实现本地自由推理输出</td>
</tr>
<tr>
<td>开发团队</td>
<td>陈雨墨团队</td>
...
直接在 LM Studio 或 Jan 中搜索 `moziAI-35B`，选择量化版本下载即可。

## 基准评测

moziAI-13.7-35B-A3B 基于 **Ornith-1.0-35B**（deepreinforce-ai）底座微调。MoziAI 在底座优秀的智能体编码能力基础上，新增**金融垂直领域深度优化**，在金融问答、量化编程、工具调用等场景下表现更出色。通用能力与 Ornith-1.0-35B 底座保持一致。
