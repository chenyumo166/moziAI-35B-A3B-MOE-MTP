...
# MoziAI-V3.6-35B-A3B-MOE - 무료 로컬 배포 가능한 소형 고성�?멀티모�?AI

Language / 언어 선택  
[简体中文](README.zh.md) | [繁體中文](README.zh-hant.md) | [日本語](README.ja.md) | 한국�?| [हिन्दी](README.hi.md) | [English](README.en.md) | [Deutsch](README.de.md) | [Français](README.fr.md) | [Nederlands](README.nl.md) | [Italiano](README.it.md) | [Русский](README.ru.md)

## 모델 개요

MoziAI-35B-A3B-MOE�?중국 금융 인플루언�?천위�?陳雨�? 팀�?개발�?로컬 오픈소스 멀티모�?AI 대�?언어 모델입니�?(금융 영역 강화, 비전 지�? 도구 호출, 복잡�?�?작업 지�? 소비�?GPU 로컬 배포). Ornith-1.0-35B-A3B(**Qwen3.5-35B-A3B/Qwen3.6-35B-A3B** 아키텍처, MIT 라이선스) 기반 모델에서 파인튜닝/증류되었습니�?

우리 팀�?미션은 강력�?로컬 AI 대�?모델�?수많은 가정과 중소기업�?가져다주는 것이�? �?이상 비싼 AI 하드웨어 비용이나 클라우드 API 비용�?지불할 필요가 없습니다. 자체 개발�?**MoziSmartBit 지�?양자�?* 기술�?통해 350�?파라미터 MoE 모델�?�?**15.5 GB**�?압축하여, 모델 정밀도와 크기 �?최적�?균형�?달성했으�? FP16�?�?99% 정밀�?품질을实现했습니�? 모델은 �?350�?파라미터�?가지�?있지�? MOE 희소 전문가 기술�?사용하여 토큰�?30�?파라미터�?활성화하�?MTP 추측 디코딩을 지원하�?추론�?가속화합니�? 실제 테스�?결과 20GB VRAM�?가�?소비�?GPU에서 로컬�?배포�?�?있으�?140+ tokens/s�?추론 속도�?달성하여 많은 유료 클라우드 AI 대�?모델보다 빠릅니다.

일반 AI 기능�?유지하는 �?외에�? �?모델은 금융 수직 영역�?애플리케이션, 금융 Q&A, 양적 프로그래�? 일반 프로그래�? 도구 호출, 256K 복잡�?�?컨텍스트 작업�?성공�?�?AI 대�?모델�?핵심 기능�?중점�?두고 최적화했습니�? 소비�?등급 GPU에서 무료�?로컬 배포�?�?있으�? 상당�?클라우드 토큰 비용�?절약하고 로컬 데이�?프라이버시와 보안�?보장하면�?7X24 토큰 자유�?달성합니�?

**출시�? 2026-08-20** | **버전: V3.6**

## 모델 다운로드

모델 파일�?크기 때문에（�?15.5 GB）여�?커뮤니티 플랫폼에 호스팅되�?있습니다�?
...
> 💡 **다운로드 �?*: �?링크�?클릭하여 HuggingFace 리포지토리�?이동�?다음, **「Files and versions�?* 탭에�?V3.6 디렉토리 아래�?모든 파일�?다운로드하세요（메인 모델, 비전 프로젝션, 채팅 템플릿）. �?파일 모두 같은 디렉토리�?배치하세�?

### ⚠️ 중요: 비전 기능에는 mmproj 파일�?필요합니�?

�?모델은 멀티모�?비전�?지원합니다. **비전 프로젝션 파일（mmproj�?*은 버전 디렉토리�?포함되어 있습니다�?

- **비전 파일**: `moziAI-V3.6-35B-uncensored-heretic-mmproj-BF16.gguf`（약 903 MB, BF16 정밀도）
- **배치 위치**: GGUF 모델 파일�?같은 버전 디렉토리�?배치
- **로드 방법**: llama-server 시작 �?`--mmproj` 플래그로 로드

> 비전 파일�?로드하지 않으�?모델�?* 이미지 이해 능력�?상실**하고 텍스�?전용 대�?기능�?남습니다.

### ⚠️ 중요: 채팅 템플�?파일�?반드�?로드하세�?

�?모델은 전용 채팅 템플릿을 사용합니�? **로드하지 않으�?대�?형식 오류, 추론 체인 손상, 응답 품질 저하가 발생합니�?** 템플�?파일은 버전 디렉토리�?포함되어 있습니다�?

- **템플�?파일**: `moziAI-V3.6-35B-chat-template.jinja`（몇 KB, jinja 형식�?
- **배치 위치**: GGUF 모델 파일�?같은 버전 디렉토리�?배치
- **로드 방법**: llama-server 시작 �?`--chat-template-file` 매개변수로 지�?

> 채팅 템플릿을 로드하지 않으�?모델�?시스�?프롬프트, 사용�?메시지, 사고 블록�?올바르게 인식하지 못해 출력 형식�?깨지거나 추론 능력�?저하될 �?있습니다.

### llama.cpp 시작 명령어（20GB 이상 GPU에서 256K 컨텍스트 활성�?권장 설정�?

> 참고: VRAM�?20GB 미만이면 `-c 262144`�?컨텍스트 크기 262144�?줄이세요.
...
HuggingFace / ModelScope 에서 V3.6 디렉토리 아래�?모든 파일�?다운로드�?

```
V3.6/
├── moziAI-V3.6-Qwen3.6-35B-A3B-Ornith-MoziSmartBit-Q4_K_M-Uncensored.gguf      # 메인 모델（필수）
├── moziAI-V3.6-35B-uncensored-heretic-mmproj-BF16.gguf  # 비전 프로젝션（옵션）
└── moziAI-V3.6-35B-chat-template.jinja                  # 채팅 템플릿（필수! 없으�?대�?형식 오류�?
```

> ⚠️ **채팅 템플릿은 필수 파일**이며 옵션�?아닙니다. �?모델에는 커스텀 대�?형식（추�?체인/사고 블록 포함）이 있으�? 템플릿이 없으�?출력 형식�?깨지�?추론 능력�?비활성화됩니�? 반드�?다운로드하여 시작 �?로드하세�?

### 2. 추론 서비�?시작

전체 권장 설정 시작 명령어는 위의 **llama.cpp 시작 명령�?* 장을 참고하세�?
...
> 비전 기능�?필요하면 `--mmproj V3.6/moziAI-V3.6-35B-uncensored-heretic-mmproj-BF16.gguf` �?추가하세�?

### 3. 사용 시작

브라우저에서 `http://localhost:8080` �?열면 채팅�?시작�?�?있습니다.
...
�?  └── moziAI-V3.6-35B-chat-template.jinja   # 채팅 템플�?
```
...
- **MoziSmartBit 지�?양자�?*: 자체 개발 스마�?양자�? 정밀도와 크기�?최적 균형, 거의 무손실로 �?**15.5 GB**�?압축
- **복잡�?�?작업 능력**: 모델 에이전트가 작업�?자동으로 계획하고, 막힌 지점을 처리하고 자기 성찰하는 지능형 순환 메커니즘으로 훈련되어, 복잡�?작업�?자동 실행�?자기 조정�?가능하�?하여, 사용자가 지속적으로 프롬프트�?최적화해�?하는 번거로움�?없앱니다.
- **작은 모델, �?능력**: 복잡�?작업에서 350�?파라미터 이내�?다른 모델보다 성능�?뛰어나며, 심지�?파라미터가 �?�?�?�?일부 모델보다�?성능�?우수합니�?
- **MOE+MTP 속도 이점**: 모델은 �?350�?파라미터�?가지�?있지�? 토큰�?8+1 전문가�?30�?파라미터�?활성화하므�?추론 속도가 �?빠릅니다. 20GB 또는 24GB VRAM�?가�?소비�?GPU에서 로컬 배포하기�?완벽하며 140+ tokens/s�?추론 속도�?제공합니�?
...
- **비전 이해**: 추론 프레임워크에�?비전 파일�?로드하여 멀티모�?비전�?지원하�? 로컬 스크린샷�?채팅 창에 직접 붙여넣어 이미지 이해 가�?
...
�?모델은 Ornith-1.0-35B 기본 모델�?**Uncensored** 기능�?계승하며, 다음�?같은 장점�?있습니다:
...
> MoziAI V3.6은 MoziSmartBit 지�?양자화를 사용하여 ~99% 정밀도를 유지하면�?35B 파라미터 MoE 모델�?~15.5 GB(~4.5�?압축�?�?압축하며, 추론 품질�?소비�?GPU 배포 간소화의 균형�?맞추�?있습니다.
...
|------|---------------|---------|-----------|------|
| 20 GB | 128K | q4_0 | 지�?| 모델+비전 ~16.4GB, 실제 테스�?�?200K+비전 사용 �?~19.5GB VRAM 소요 |
...
MoziAI�?**deepreinforce-ai/Ornith-1.0-35B**에서 파인튜닝되었습니�? MoziAI�?기본 모델 위에�?금융 수직 영역�?최적화되�? 금융 Q&A, 양적 프로그래�?�?도구 호출 시나리오에서 우수�?성능�?제공합니�? MoziAI-35B�?일반 기능은 Ornith-1.0-35B 기본 모델�?일치합니�?
...
> MoziAI-35B�?일반 벤치마크 점수�?Ornith-1.0-35B 기본 모델�?일치합니�? 금융 수직 영역은 MoziAI�?핵심 최적�?방향으로, 재무제표 분석, 양적 전략, 리스�?�?컴플라이언스, 에이전트 도구 호출 시나리오에서 일반 모델�?크게 능가합니�? Gemma4 �?Qwen3.6 데이터는 공식 공개 결과에서 가져왔습니�?

## SEO 키워�?

financial AI LLM, 로컬 오픈소스 모델, 엔드사이�?모델, 양적 프로그래�? MoziSmartBit, 지�?양자�? GGUF 양자�? MoE 모델, 로컬 오픈소스 LLM, 로컬 배포, 금융 AI, 도구 호출, Agent, llama.cpp, Ollama, GGUF, Uncensored, 검�?없음, 자유 출력, 제한 없음, Q3_K_M, Q4_K_M, Q5_K_M, Q6_K, Q8_0, Ornith-1.0-35B, Qwen3.5, Qwen3.6, 금융 수직 영역, 오픈소스 모델
