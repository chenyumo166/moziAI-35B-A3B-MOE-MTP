...
[中文](README.md) | [English](README.en.md) | [日本語](README.ja.md) | 한국어 | [Deutsch](README.de.md) | [Français](README.fr.md) | [Русский](README.ru.md) | [हिन्दी](README.hi.md) | [繁體中文](README.zh-hant.md) | [Nederlands](README.nl.md) | [Italiano](README.it.md)

## 모델 개요

MoziAI-35B-A3B-MOE는 중국 금융 인플루언서 천위모(陳雨墨) 팀이 개발한 로컬 오픈소스 금융 AI 멀티모달 LLM(비전 및 도구 호출 지원)으로, Ornith-1.0-35B(**Qwen3.5-35B-A3B / Qwen3.6-35B-A3B** 아키텍처, MIT 라이선스) 기반 모델에서 파인튜닝/증류되었습니다. 자체 개발한 **MoziSmartBit 지능 양자화** 기술을 통해 35B 파라미터 MoE 모델을 약 **15.5 GB**로 압축하여, 손실 수준 ~99% 정밀도 품질로 정밀도와 크기 간 최적의 균형을 달성했습니다.
...
- **멀티에이전트 플랫폼 지원**: OpenClaw, Hermes, OpenCode, Cursor, Windsurf, Claude Code, Codex 및 기타 주류 AI IDE 및 에이전트 프레임워크와 심층 통합, 도구 호출 및 멀티턴 작업 오케스트레이션을 네이티브 지원, 바로 사용 가능
...
| 장점 | 설명 |
|------|------|
| **검열 없음** | 민감하거나 논쟁적인 콘텐츠를 포함한 모든 주제를 거부하지 않음 |
| **자유 출력** | 안전 정책의 제한을 받지 않고, 모든 유형의 응답을 생성할 수 있음 |
| **완전한 정보** | 필터링되지 않은 완전한 정보를 제공하여, 연구 및 분석에 적합 |
| **로컬 프라이버시** | 로컬 배포로 데이터가 완전히 비공개이며, 클라우드 검열로부터 자유로움 |

> **사용 사례**: 학술 연구, 심층 분석, 자유 논의, 제한 없는 AI 대화.
> **참고**: 이 모델은 로컬 배포 모델이며, 출력 콘텐츠는 사용자가 완전히 제어합니다. 콘텐츠 관리 책임이 없습니다.

## 핵심 기능

| 기능 영역 | 설명 |
|-----------|------|
| 시장 분석 | 거시/미시경제 해석, A주/HK/미국 주식/원자재/암호화폐 시장 논리 |
| 재무 보고서 | 주요 재무 지표 해석, 리서치 보고서 요약, 밸류에이션 및 실적 전망 지원 |
| 리스크 및 컴플라이언스 | 상품 리스크 평가, 투자 권고 컴플라이언스, 금융 규제 정책 해석 |
| 양적 전략 | 양적 전략 설계, Pyramid(PEL) 양자화, 백테스팅 로직, 팩터 구축 및 도구 호출 |
| 도구 호출 | 실시간 시세, 데이터베이스, 리서치 보고서 검색 및 기타 금융 데이터 소스 통합 |
...
| 최소 VRAM | 20GB+ VRAM 소비자 GPU (예: RTX 4060 Ti 16G CPU 오프로드 사용 시), 24 GB 권장 (비전 + 긴 컨텍스트) |
| 추론 프레임워크 | llama.cpp / Ollama / LM Studio / Jan |
| 추론 속도 | 알고리즘 최적화: AMD R700 GPU에서 140+ token/s, AMD MAX+395 CPU iGPU에서 70+ token/s, 로컬 토큰 자유 |
| 팀 | 천위모 팀 |
...
| **MoziSmartBit** | **~15.5 GB** | **~99%** | **MoziAI가 사용하는 최적의 양자화 방식** |
| Q4_K_M | ~21.2 GB | ~98% | GGUF 표준 4bit |
| Q5_K_M | ~24.7 GB | ~99% | 더 높은 품질 |
...
기존 양자화는 모든 레이어에 균일한 정밀도를 적용합니다. **MoziSmartBit 지능 양자화**는 최적의 크기-정밀도 균형을 위해 차별화된 양자화 전략을 적용합니다.

### 압축 효과

기존 양자화는 모델의 모든 부분을 균일하게 압축하여 종종 상당한 정밀도 손실을 초래합니다. MoziSmartBit 지능 양자화는 자체 개발한 지능 압축 전략을 사용하여 **최소한의 정밀도 손실로 상당한 크기 축소를 달성**합니다:

- **최소 양자화 손실**: 훈련 효과 > 양자화 손실. 훈련된 MoziAI-35B는 금융 영역 텍스트에서 사전 훈련 bf16 기본 모델보다 더 나은 PPL을 달성하며, 유사 AI 모델 대비 환각과 혼란도(perplexity)를 줄입니다
- **~4.5배 크기 축소**: ~70 GB(FP16)에서 ~15.5 GB로 압축, Q4_K_M(~21 GB)보다도 크게 작아 VRAM 및 저장소 요구 사항을 크게 낮춤
- **소비자 GPU 친화적**: 이전에 고급 GPU가 필요했던 35B MoE 모델이 이제 20GB~24GB VRAM에서 원활하게 실행 가능

### 비교 우위

**Q4_K_M(~21.2 GB) 대비**: ~27% 더 작음(~15.5 GB), 정밀도는 Q4_K_M보다 **높음**, VRAM 진입 장벽이 더 낮음 — 중급 소비자 GPU(24GB)에서 원활하게 실행 가능.

**FP16 원본(~70 GB) 대비**: ~4.5배 압축, 훈련 효과 + 최소한의 양자화 손실(훈련 효과 > 양자화 손실), 전문가급 하드웨어 대신 소비자 GPU에서 로컬 256K 컨텍스트 배포 가능.
...
사용자 GPU 구성이 각기 다르므로, VRAM 크기에 따른 권장 매개변수를 안내합니다 (모두 MoziSmartBit 버전 기준):
...
> 💡 **팁**: VRAM이 위 요구 사항을 충족하는 한 작동합니다. 브랜드나 모델 제한이 없습니다. NVIDIA / AMD / Intel 독립 GPU 및 위 목록의 128GB 통합 메모리 iGPU를 지원합니다.

> 💡 **팁**: 더 긴 컨텍스트는 더 많은 VRAM을 사용합니다. OOM(메모리 부족)이 발생하면 `-c` 값을 점진적으로 줄이세요. `--fit on`을 사용하면 llama.cpp가 VRAM에 맞게 자동으로 레이어를 조정합니다.
...
LM Studio 또는 Jan에서 `moziAI-35B`를 검색하고, MoziSmartBit 양자화 버전을 다운로드하세요.
...
|----------|----------------------|-------------|------------|------------|-------------|------|
| Terminal-Bench 2.1 | 64.2 | 59.3 | 42.1 | - | 41.4 | 자율 터미널 코딩 |
| Terminal-Bench (Claude Code) | 62.8 | 59.3 | - | - | 38.9 | Claude Code 코딩 |
| SWE-bench Verified | 75.6 | 77.2 | 52.0 | - | 70.0 | 실제 소프트웨어 엔지니어링 |
| SWE-bench Pro | 50.4 | 53.5 | 35.7 | - | 44.6 | 복잡한 소프트웨어 엔지니어링 |
| SWE-bench Multilingual | 69.3 | 71.3 | - | - | 60.3 | 다국어 코딩 |
| NL2Repo | 34.6 | 36.2 | 15.5 | - | 20.5 | 자연어를 저장소로 변환 |
| LiveCodeBench v6 | 63.3 | 83.9 | 80.0 | 77.1 | - | 경쟁 프로그래밍 |
...
모델 크기가 크기(~15.5 GB) 때문에, 가중치는 여러 커뮤니티 플랫폼에 호스팅됩니다:
...
전체 권장 구성은 위의 [llama.cpp 실행 명령어](#llamacpp-실행-명령어)를 참조하세요.
...
브라우저에서 `http://localhost:8080`을 열고 채팅을 시작하세요.
...
이 모델은 **커스텀 제한 라이선스**를 사용합니다:

### ✅ 허용
- **무료 상업적 사용**: 상업 제품에 자유롭게 통합 가능
- **복사 및 배포**: 복사, 다운로드, 공유 가능

### ❌ 금지
- **파생 작품**: 모델 또는 그 일부를 수정, 번역, 적응, 병합 또는 파인튜닝 금지
- **재판매**: 모델을 단독으로 또는 제품의 일부로 판매 금지
- **재라이선스**: 서브라이선스 부여 금지
...
있는 그대로 제공되며 보증이 없습니다. 모델 출력은 참고용이며 투자 권유가 아닙니다. 사용자가 모든 리스크를 부담합니다.
