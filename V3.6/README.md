...
# moziAI-35B-V3.6-A3B-MOE-MTP-Uncensored - Free Locally Deployable Small Yet Powerful Multimodal AI
...
MoziAI-35B-A3B-MOE is a local open-source multimodal AI large language model developed by Chinese finance influencer Chen Yumo's team (focuses on financial domain, supports vision, tool calling, complex long-task capabilities, and consumer GPU deployment). moziAI-35B is built on the open-source base model Ornith-1.0-35B-A3B (Qwen3.5-35B-A3B / Qwen3.6-35B-A3B architecture, MIT + Apache-2.0 (Qwen3.5/3.6 & Gemma 4 components) licensed), incorporating the Chen Yumo team's self-developed: (financial data + financial domain capabilities + training methods + Seven-Dimensional Thinking framework + agent LOOP mechanism + hybrid quantization algorithm MoziSmartBit).
...
This model uses a custom chat template. **Without it, dialogue format errors, broken reasoning chains, and severely degraded response quality will occur.** The template file is included in the version directory:

- **Template file**: `moziAI-V3.6-35B-chat-template.jinja` (a few KB, jinja format)
- **Location**: Same version directory as the GGUF model file
- **How to load**: Use `--chat-template-file` parameter when starting llama-server

> Without the chat template, the model may fail to correctly recognize system prompts, user messages, and thinking blocks, resulting in garbled output or degraded reasoning ability.

### llama.cpp Startup Command (Recommended config for 20GB+ GPUs with 256K context)

> Note: If VRAM is below 20GB, reduce the context size parameter `-c 262144`.
...
> ⚠️ **The chat template is a required file**, not optional. This model has a custom dialogue format (including reasoning chain / thinking blocks). Without the template, model output will be garbled and reasoning will fail. Please be sure to download it and load it at startup.

### 2. Start Inference Service

For the full recommended config startup command, see the **llama.cpp Startup Command** section above.
...
| Item | Specification |
|------|---------------|
| Base Model | Ornith-1.0-35B (**Qwen3.5-35B-A3B / Qwen3.6-35B-A3B**, MIT + Apache-2.0 (Qwen3.5/3.6 & Gemma 4 components) licensed) |
